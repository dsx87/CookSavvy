...
///
/// Conforming types must load the catalog on demand, expose FTS5-backed search,
/// category browsing, and exact-name lookup. The protocol allows `IngredientsService`
/// to be replaced with a mock in tests and DEBUG builds.
protocol IngredientsServiceProtocol: AnyObject {
    /// Ensures the ingredient catalog has been imported into the database; imports on first call.
    ...
    /// Forces a fresh re-import of the ingredient catalog, replacing all existing entries.
